...
impl Packet {
    ...
    pub fn calc_tcp_checksum(&mut self) -> Result<u16, csum::TransportCalcError> {
            ...
            EtherType::Ipv4 => {
                ...
                match ipv4.proto {
                    IpProto::Tcp => {
                        ...
                        let pseudo_seed = csum::calculate_ipv4_pseudo_header_checksum(ipv4, (self.len() - offset) as u32 + IpProto::Tcp as u32);
                        (pseudo_seed, tcp_hdr)
                    }
                    proto => return Err(csum::TransportCalcError::NotTcp(proto)),
                }
            }
            EtherType::Ipv6 => {
                ...
                match ipv6.next_header {
                    IpProto::Tcp => {
                        let tcp_hdr = self.read::<TcpHdr>(offset)?;
                        let pseudo_seed = csum::calculate_ipv6_pseudo_header_checksum(ipv6, (self.len() - offset) as u32 + IpProto::Tcp as u32);
                        (pseudo_seed, tcp_hdr)
                    }
                    proto => return Err(csum::TransportCalcError::NotTcp(proto)),
                }
            }
            invalid => return Err(csum::TransportCalcError::NotIp(invalid)),
        };

        let checksum = if self.can_offload_checksum() {
            let csum = csum::fold_checksum(pseudo_seed);
            ...
            tcp_hdr.checksum = 0;
            let sum = csum::partial(tcp_hdr.as_bytes(), pseudo_seed);

            let data_offset = offset + TcpHdr::LEN;
            let data_payload = &self[data_offset..self.len()];

            let mut csum = csum::fold_checksum(csum::partial(data_payload, sum));
